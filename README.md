...
[![Jules.ai Compatible](https://img.shields.io/badge/Jules.ai-Compatible-purple.svg)](https://jules.google.com)
...
Copy and paste the prompt below into any AI agent (Jules, Cursor, Copilot, ChatGPT, etc.) to scan this repo and set up curated prompts for your codebase in seconds.

```
Scan this repository and do the following:
...
2. UNDERSTAND the codebase — scan package.json, config files, source directories, and dependencies to identify the tech stack, frameworks, and project structure.
...
After the initial 5, import these based on your stack:

| Your Stack | Import These |
|---|---|
...
└── auto-generated-ai-agents-by-jules/  # Jules-generated content
    ...
    └── enhanced-agents/      # Draft improvements (staged for merging)
```
...
### 2. Integration with [Jules by Google](https://jules.google.com)

1. **Copy the agent prompt** from the corresponding `.md` file
2. **Create a new agent** in your [Jules dashboard](https://jules.google.com)
...
- Enhances agents in-place (single source of truth per agent)
- Adds new patterns, examples, and best practices to existing prompts
...
| Token Efficiency | > 85% | 88.5% | Useful token output vs context window usage |
| Time to Resolution | < 2 hrs | ~45 mins | Average time from issue creation to merged PR |
...
### 📁 What Jules Creates
- **Enhanced Agents** - In-place improvements to existing agent prompts
...
├── daily-updates/        # Daily improvements by Jules
├── new-agents/          # New agents created by Jules
└── enhanced-agents/      # Draft improvements (staged before merging into root files)
```
...
- [**TODOist — Todo File Generator**](TODOist.md) - 7-phase codebase audit that generates 11 prioritized todo files + a ready-to-use execution prompt
...
- **[Jules by Google](https://jules.google.com)** - Platform for autonomous agent deployment
